...
install-cert-manager: ## Install cert manager onto the target kubernetes cluster
	set -e ;\
	curl -Ls $(CERT_MANAGER_URL) | sed 's/quay.io/localhost:5001/g' | kubectl apply -f - ;\
...
new-daemon-images:
	make daemon && make docker-build && make docker-push && make docker-build-daemon && make docker-push-daemon
